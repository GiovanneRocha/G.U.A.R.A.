...
from utils import draw_text, clamp, draw_grid_background
...
class Game:
    def __init__(self, record_demo_seconds:int|None=None):
        ...
        if self.autoplay and Image is None:
            print("Pillow não disponível. O GIF não será salvo.")

    def reset_level(self):
        cfg = LEVELS[self.level_index]
        self.manager = TypingManager(cfg.words, speed_factor=cfg.speed_factor,
                                     boss_words=(cfg.boss_words if cfg.has_boss else None),
                                     boss_health=(cfg.boss_health if cfg.has_boss else 0))
        self.score = 0
        self.hits = 0
        self.misses = 0
        ...
        self.wpm_timer = 0.0
        self.wpm_words = 0
        self.corruption = 25.0
        self.spawn_t = 0.0
        self.spawn_interval = max(S.MIN_SPAWN_INTERVAL, S.BASE_SPAWN_INTERVAL / max(0.1, LEVELS[self.level_index].spawn_factor))
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == KEYDOWN:
                if ev.key == K_ESCAPE:
                    ...
                    self.state = GameState.MENU
                elif self.state == GameState.PLAY and not self.autoplay:
                    ch = ev.unicode.lower()
                    if ch and ch.isprintable() and len(ch) == 1 and ch.isalnum():
                        ok, cleared, boss_hit = self.manager.handle_char(ch)
                        if ok:
                            self.hits += 1
                            assets.play("ok")
                            if boss_hit:
    ...
    def update(self, dt):
        ...
        # Autoplay typing (for demo)
        if self.autoplay:
            ch = self.manager.autoplay_step(dt)
            if ch:
                ok, cleared, boss_hit = self.manager.handle_char(ch)
                if ok:
                    self.hits += 1
        ...
        self.spawn_t += dt
        if cfg.has_boss:
            # durante boss, spawna poucos minions
            if self.manager.boss_should_spawn_minion():
                self.manager.reset_boss_minion_timer()
        ...
        # vitória/derrota
        if cfg.has_boss:
            if self.manager.boss and self.manager.boss.defeated:
    ...
    def draw_hud(self):
        ...
        draw_text(self.screen, "Corrupcao da Rede", 16, 90, S.FONT_SMALL, (200,200,220))

    def draw_menu(self):
        draw_grid_background(self.screen, (16,16,22), (28,30,42))
        draw_text(self.screen, "G.U.A.R.A. - Guardiao Unificado de Ambientes de Rede e Aprendizagem", S.WIDTH//2, 140, 22, (180,220,255), center=True)
        draw_text(self.screen, "GuaraByte: Protocolo ODS", S.WIDTH//2, 190, 42, (120, 230, 180), center=True)
        draw_text(self.screen, "Pressione qualquer tecla para iniciar", S.WIDTH//2, 260, 24, (210,210,230), center=True)
        draw_text(self.screen, "Mecanica: digite as palavras nos 'pacotes corrompidos' para depurar a rede.", S.WIDTH//2, 320, 20, (200,200,220), center=True)
        draw_text(self.screen, "ESC: sair (ou voltar ao menu durante o jogo).", S.WIDTH//2, 360, 18, (160,160,180), center=True)

    def draw_victory(self):
        draw_grid_background(self.screen, (8, 30, 20), (14,44,28))
        draw_text(self.screen, "Rede CPS Restaurada!", S.WIDTH//2, 200, 42, (140, 255, 190), center=True)
        draw_text(self.screen, "Parabens, G.U.A.R.A.! Missao Cumprida.", S.WIDTH//2, 260, 24, (220, 240, 230), center=True)
    ...
    def draw_defeat(self):
        draw_grid_background(self.screen, (30, 10, 16), (42,18,28))
    ...
    def capture_frame(self):
        if self.record_seconds is None or Image is None:
            return
        raw_str = pygame.image.tostring(self.screen, 'RGB')
        image = Image.frombytes('RGB', self.screen.get_size(), raw_str)
        self.record_frames.append(image)

    def save_gif(self):
        if self.record_seconds is None or Image is None:
            return
        os.makedirs(S.DEMO_DIR, exist_ok=True)
        out = os.path.join(S.DEMO_DIR, 'demo.gif')
        duration = int(1000/60)  # ms per frame ~60fps
        # To keep file small, sample every 2 frames
        frames = self.record_frames[::2]
        if frames:
            frames[0].save(out, save_all=True, append_images=frames[1:], duration=duration*2, loop=0)
    ...
    def run(self):
        ...
        while True:
            ...
            # end demo recording
            if self.record_seconds is not None:
                elapsed += dt
