...
class Game:
    def __init__(self, record_demo_seconds: int | None = None):
        # Choose portrait window size based on current display (fit within screen)
        pygame.init()
        info = pygame.display.Info()
        max_h = int(info.current_h * 0.92)
        h = max(min(max_h, 960), 720)  # clamp useful portrait height
        w = int(h * 9/16)
        ...
        self.current_input = ""
        self.spawn_t = 0.0
        self.spawn_interval = 1.0 # will be reset per level
    ...
    def begin_level(self, i: int):
        ...
        self.current_input = ""
        self.spawn_t = 0.0
        # spawn interval based on base interval and factor
    ...
    def handle_key_char(self, ch: str):
        ...
        # If boss active & starting a new word, decide target by first char
        if (self.boss and not self.boss.defeated) and (not self.current_input) and (self.target_enemy is None):
            ...
            if candidate:
                ...
                ok_b, _ = self.boss.handle_char(ch)
                if ok_b:
                    assets.play('ok')
                self.current_input += ch
        else:
        ...
        while running:
            ...
            for ev in pygame.event.get():
                if ev.type == QUIT:
                    ...
                    if ev.key == K_ESCAPE:
                        running = False
                    elif not (self.game_over or self.victory):
                        ch = ev.unicode.upper()
